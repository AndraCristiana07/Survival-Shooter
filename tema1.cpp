...
bool Tema1::IsBallSquareCollision(float ballx, float ballY, float radius, float squareX, float squareY, float sizeX,
                                  float sizeY) {
...
void Tema1::Init() {
    ...
    camera->SetOrthographic(0, (float) resolution1.x, 0, (float) resolution1.y, 0.0001f, 400);
    ...
    for (int i = 0; i < nr_obj; i++) {
        ...
        while (IsBallBallCollision(translateX, translateY, radiusCircle, objX + x * scaleX, objY + y * scaleY,
                                   max(x * scaleX, y * scaleY))) {
            objX = rand() % (MAX_MAP - 1);
            objY = rand() % (MAX_MAP - 1);
    ...
    Mesh *healthBar = object2D::CreateSquare("healthBar", cornerSquare, healthBarX, healthBarY, glm::vec3(1, 0, 0),
                                             false);
    AddMeshToList(healthBar);

    Mesh *healthLvl = object2D::CreateSquare("healthLvl", cornerSquare, healthLvlX, healthLvlY, glm::vec3(0.1, 0.9, 0),
                                             true);
...
void Tema1::Update(float deltaTimeSeconds) {
    ...
    if (pickUpsTime < 0) {
        ...
        for (int i = 0; i < objects.size(); i++) {
            while (IsBallSquareCollision(p.x - pickUpRadius, p.y - pickUpRadius, pickUpRadius, objects[i].x,
...
void Tema1::DrawScene(glm::mat3 visMatrix) {
    ...
    modelMatrix = visMatrix * transform2D_t1::Translate(translateX + 3 * logicSpace.width / 4 - offset_healthBar,
                                                        translateY - logicSpace.width / 2 + offset_healthBar);
    RenderMesh2D(meshes["healthBar"], shaders["VertexColor"], modelMatrix);

    modelMatrix = glm::mat3(1);
    modelMatrix = visMatrix * transform2D_t1::Translate(translateX + 3 * logicSpace.width / 4 - offset_healthBar,
                                                        translateY - logicSpace.width / 2 + offset_healthBar);
    modelMatrix *= transform2D_t1::Scale(1, scaleY_healthLvl);
    RenderMesh2D(meshes["healthLvl"], shaders["VertexColor"], modelMatrix);


    righteye = glm::mat3(1);
    righteye *= visMatrix * transform2D_t1::Translate(translateX - offset_eye, translateY - offset_eye);
    righteye *= transform2D_t1::Rotate(angularStep + M_PI / 2);
    RenderMesh2D(meshes["pupil"], shaders["VertexColor"], righteye);

    lefteye = glm::mat3(1);
    lefteye *= visMatrix * transform2D_t1::Translate(translateX - offset_eye, translateY - offset_eye);
    lefteye *= transform2D_t1::Rotate(angularStep);
    ...
    body = glm::mat3(1);
    body *= visMatrix * transform2D_t1::Translate(translateX, translateY);
    RenderMesh2D(meshes["body"], shaders["VertexColor"], body);

    for (auto &enemy: enemies) {
        if (!enemy.remove) {
            righteye = glm::mat3(1);
            righteye *= visMatrix * transform2D_t1::Translate(enemy.x - offset_eye, enemy.y - offset_eye);
            righteye *= transform2D_t1::Rotate(enemy.alpha + M_PI / 2);
            RenderMesh2D(meshes["enemy_pupil"], shaders["VertexColor"], righteye);


            lefteye = glm::mat3(1);
            lefteye *= visMatrix * transform2D_t1::Translate(enemy.x - offset_eye, enemy.y - offset_eye);
            lefteye *= transform2D_t1::Rotate(enemy.alpha);
            ...
            modelMatrix = glm::mat3(1);
            modelMatrix = visMatrix * transform2D_t1::Translate(enemy.x, enemy.y);
    ...
    for (int i = 0; i < nr_obj; i++) {
        modelMatrix = glm::mat3(1);
        modelMatrix *= visMatrix * transform2D_t1::Translate(objects[i].x, objects[i].y);
        modelMatrix *= transform2D_t1::Scale(objects[i].scaleX, objects[i].scaleY);
    ...
    for (auto &bullet: bullets) {
        if (!bullet.remove) {
            modelMatrix = glm::mat3(1);
            modelMatrix = visMatrix * transform2D_t1::Translate(bullet.x, bullet.y);
    ...
    for (auto &pickUp: pickUps) {
        if (!pickUp.remove) {
            modelMatrix = glm::mat3(1);
            modelMatrix = visMatrix * transform2D_t1::Translate(pickUp.x, pickUp.y);
            RenderMesh2D(meshes["pick_up"], shaders["VertexColor"], modelMatrix);

            modelMatrix = glm::mat3(1);
            modelMatrix = visMatrix * transform2D_t1::Translate(pickUp.x + 0.05f, pickUp.y + 0.05f);
    ...
    modelMatrix = glm::mat3(1);
    modelMatrix = visMatrix * transform2D_t1::Translate(0, 0);
...
void Tema1::OnInputUpdate(float deltaTime, int mods) {
    ...
    for (auto &enemy: enemies) {
        x1 = enemy.x - translateX;
        y1 = enemy.y - translateY;
        enemy.alpha = atan2(y1, x1) - (float) M_PI / 2;
        enemy.x -= (float) deltaTime * enemy.speed * (float) cos(enemy.alpha + M_PI / 2);
        enemy.y -= (float) deltaTime * enemy.speed * (float) sin(enemy.alpha + M_PI / 2);
    }

    for (auto &pickUp: pickUps) {
        if (pickUp.remove == false) {
            if (IsBallBallCollision(pickUp.x, pickUp.y, pickUpRadius,
    ...
    for (int i = 0; i < enemies.size(); i++) {
        if (enemies[i].remove == false) {
            if (IsBallBallCollision(enemies[i].x, enemies[i].y, radiusCircle, translateX, translateY, radiusCircle)) {
    ...
    for (int i = 0; i < enemies.size(); i++) {
        if (enemies[i].remove == false) {
            if (0 >= enemies[i].x - (float) radiusCircle - offset_body ||
                enemies[i].x + offset_body >= MAX_MAP ||
                0 >= enemies[i].y - radiusCircle - (float) offset_body ||
                enemies[i].y + offset_body >= MAX_MAP) {
                enemies[i].remove = true;
            }
        }
                    ...
                    for (int i = 0; i < nr_obj; i++) {
                        ...
                        if (!IsBallSquareCollision(bullets[k].x, bullets[k].y, radiusBullet, objects[i].x,
                            ...
                            for (int a = 0; a < enemies.size(); a++) {
                                if (enemies[a].remove == false) {

                                    if (IsBallBallCollision(bullets[k].x, bullets[k].y, radiusCircle,
                                                            enemies[a].x, enemies[a].y, radiusBullet)) {
                                        score += 10;
                                        cout << "Score: " << score << "\n";
                                        bullets[k].remove = true;
                                        enemies[a].remove = true;
                                    }
                                }
